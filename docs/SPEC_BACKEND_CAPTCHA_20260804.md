...
브라우저로 캡차를 직접 풀어 토큰을 받고, 백엔드가 할 호출을 실제로 때려봤다.

### ① `session_id` 는 캡차가 만든다

```
sessionStorage["captcha-session"] = "b0403b61-8039-4224-a0da-43abceb306bc"
```

캡차가 이 UUID 를 만들어 두고 verify 요청에 실어 보낸다. **백엔드는 이 값을 프론트에서
같이 받아야 한다.** 백엔드가 자체 세션 ID 를 넣으면 발급 때 값과 달라 검증이 실패한다.

### ② 토큰 수명은 120초다

```json
{"success": true, "captcha_token": "...", "expires_in": 120}
```

**푼 뒤 2분 안에 백엔드가 검증해야 한다.** 캡차를 푼 다음 다른 화면을 거쳐 오는
흐름이면 만료된다. 사용자가 잠깐 멈추기만 해도 죽는 길이다.

### 확인 못 한 것 하나

`X-Captcha-Site-Secret` 의 실제 값이 없어 마지막 호출은 401 에서 멈췄다.

```
헤더 없음                     401 Invalid site secret
틀린 값                       401
기본값 private-demo-secret     401     ← 실제 값이 따로 설정돼 있다는 뜻
```

기본값이 아니라는 것은 확인됐다. **캡차 쪽 시크릿은 제대로 설정돼 있다.** 값만 있으면
왕복이 완결된다.
...
① 캡차 로드          ✅
② 문항 풀이           ✅
③ 토큰 발급           ✅   captcha_token · 120초
④ 백엔드 검증 호출     ⛔   시크릿 값만 있으면 통과
```
