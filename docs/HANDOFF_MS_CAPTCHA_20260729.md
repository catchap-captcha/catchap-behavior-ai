...
### ⚠️ 배포가 급해진 이유 — 지금 배포본은 정답만 채점합니다 (17:50 확인)

키 정렬 후 연동은 정상 확인했습니다(실제 풀이 → 배치 저장 → predict 200 → DB 기록,
`human_probability 1.000000`). 그런데 **풀이의 상당수가 AI에 도달하지 않습니다.**

한 변수씩 바꿔 6회 측정한 결과입니다.

```
success=True  →  AI 도달 2/2
success=False →  AI 도달 0/4
```

즉 **정답을 맞힌 풀이만 AI로 갑니다. 틀린 풀이는 점수를 내지 않고 버려집니다.**
(선택 개수·객체 수·배치 개수는 무관했습니다. 정답 여부만 갈랐습니다.)

`sw-captcha` 는 정답 여부와 무관하게 채점합니다 — `behavior_ai.score()` 가
정답 분기(`if not correct: return`) **앞**에 있고, 그걸 테스트로 고정해뒀습니다.

**이게 데이터 수집에 치명적인 이유**: 사람 오탐률(FRR)을 재려면 사람이 **틀린**
시도도 필요합니다. 그리고 봇은 애초에 정답률이 낮은데, 틀린 봇 시도가 전부
기록되지 않으면 봇 표본이 구조적으로 사라집니다. 지금 상태로 데이터를 쌓으면
**"정답 맞힌 사람"만 모인 편향된 표본**이 됩니다.
