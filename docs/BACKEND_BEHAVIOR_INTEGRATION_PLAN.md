# ⚠️ 대체됨 — `RISK_TRIGGERED_CAPTCHA_PLAN.md` 를 보라

메인 캡차가 **김민서 드래그 캡차**로 확정되어(2026-07-29) 이 이식 계획은 불필요해졌다.
그 캡차에는 행동 신뢰 프로토콜이 이미 구현돼 있고(`sw-captcha`, 테스트 23 통과),
캡차의 `POST /api/verify-token` 이 인강 서버용 접점으로 이미 존재한다.
아래는 백엔드 자체 캡차의 취약 구조 분석으로서만 참고 가치가 있다.

---

# 백엔드 캡차에 행동 신뢰 프로토콜 이식 — 계획 (경로 B, 폐기)

작성: 조성원 (행동 AI 담당) / 2026-07-29
